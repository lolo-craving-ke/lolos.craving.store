...
function productData(formData: FormData, imageUrl: string | null): Prisma.ProductUncheckedCreateInput {
  const saleType: SaleType = String(formData.get('saleType') || 'QUANTITY') === 'WEIGHT' ? SaleType.WEIGHT : SaleType.QUANTITY;
  ...
  const price = saleType === SaleType.WEIGHT ? price1kg || rawPrice || 1 : piecePrice || rawPrice || 1;
