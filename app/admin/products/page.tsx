...
export default async function ProductsAdminPage({ searchParams }: { searchParams: { error?: string; success?: string } }) {
  const [products, categories] = await Promise.all([
    prisma.product.findMany({ include: { category: true }, orderBy: [{ sortOrder: 'asc' }, { createdAt: 'desc' }] }),
    ...
    <div>
      ...
      <form action={createProduct} className="admin-card mt-8 grid gap-5 p-6" encType="multipart/form-data">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div><label className="label">Fallback price KSH</label><input name="price" type="number" className="input mt-2" placeholder="Optional" /></div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <div><label className="label">Sort order</label><input name="sortOrder" type="number" defaultValue="0" className="input mt-2" /></div>
        </div>

        <div className="rounded-2xl bg-[#fbf4e8] p-4">
          <h3 className="font-black">If sold by quantity</h3>
          <div className="mt-3"><label className="label">Piece / box price</label><input name="piecePrice" type="number" className="input mt-2" /></div>
        </div>

        <div className="rounded-2xl bg-[#fbf4e8] p-4">
          <h3 className="font-black">If sold by weight</h3>
          <div className="mt-3 grid gap-4 md:grid-cols-3">
            <div><label className="label">Quarter Kg price</label><input name="price250g" type="number" className="input mt-2" /></div>
            <div><label className="label">Half Kg price</label><input name="price500g" type="number" className="input mt-2" /></div>
            <div><label className="label">1 Kg price</label><input name="price1kg" type="number" className="input mt-2" /></div>
          </div>
        </div>
          ...
          <div key={product.id} className="admin-card flex gap-4 p-4">
            ...
            <div className="flex-1">
              ...
              <div className="mt-4 flex gap-3">
                ...
                <Link href={`/admin/products/${product.id}`} className="text-sm font-black text-[#9b6128] underline">Edit</Link>
