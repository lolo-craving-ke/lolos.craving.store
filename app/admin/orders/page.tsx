...
export default async function OrdersAdminPage({ searchParams }: { searchParams: { success?: string; error?: string } }) {
  ...
  return (
    <div>
      <div>
        ...
        <p className="mt-3 text-[#746b78]">Review, update or delete customer orders.</p>
      </div>

      {searchParams.success && <div className="mt-6 rounded-xl border border-green-200 bg-green-50 p-4 text-sm font-black text-green-700">Order deleted successfully.</div>}
      {searchParams.error && <div className="mt-6 rounded-xl border border-red-200 bg-red-50 p-4 text-sm font-black text-red-700">Could not delete order.</div>}
          ...
          <article key={order.id} className="admin-card p-6">
            <div className="flex flex-col justify-between gap-4 md:flex-row">
              ...
              <div className="text-left md:text-right">
                ...
                <h3 className="text-3xl font-semibold">{money(order.total)}</h3>
              </div>
            </div>

            <div className="mt-5 rounded-xl bg-[#faf7f2] p-4">
              {order.items.map((item) => (
                <div key={item.id} className="flex justify-between border-b border-[#e8e1ea] py-2 text-sm last:border-0">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-[1fr_160px]">
              <form action={updateOrder} className="grid gap-3 md:grid-cols-[1fr_1fr_160px]">
              ...
              <form action={deleteOrder}>
                <input type="hidden" name="id" value={order.id} />
                <button className="w-full rounded-xl border border-red-200 bg-red-50 px-5 py-3 text-sm font-black text-red-600 transition hover:bg-red-100">
                  Delete
                </button>
              </form>
            </div>
